...
class UaManagerFboot(peer.UaPeer):
    ...
    def __init__(self, address, port):
        ...
        self.port = port
        self.fboot_path = os.path.join(os.path.dirname(
            sys.path[0]), 'resources', 'data_model.fboot')
    ...
    def __call__(self, config):
        ...
        self.create_object(2, self.base_name,
    ...
    def from_fboot(self):
        ...
        try:
            ...
            if os.stat(self.fboot_path).st_size == 0:
                logging.warning(
                    'Fboot definition file is empty. Awaiting deployment')
            else:
                try:
                ...
                if xml_element.get('Action') == 'CREATE':
                    for child in xml_element:
                        if child.tag == 'FB' and child.get('Type') != 'EMB_RES':
                            root_path = utils.get_fb_files_path(
                                child.get('Type'))
                            # Check fbt file
                            fb_file = open(os.path.join(
                                root_path, '{0}.fbt'.format(child.get('Type'))), 'r')
                        ...
                        if child.tag == 'Connection':
                            if len(self.method_names) == 0 or not utils.any_element_in_string(self.method_names, child.get('Destination')):
                                ...
                                if info_type == 'INPUT':
                                    ...
                                    self.opcua_method_name = child.get(
                                        'Source')
            except KeyError:
                raise self.InvalidFbootState
    ...
    def parse_fbt(self, fb_name, file):
        ...
        if xml_root.get('OpcUa') == 'METHOD':
            ...
            item = ua_object.UaObject(self, self.folders.get(
                'FunctionBlocks'), fb_name, xml_root)
