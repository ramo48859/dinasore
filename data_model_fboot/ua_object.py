...
class UaObject:
    ...
    def __init__(self, ua_server, ua_folder, fb_resource: FBResources, fb_name: str):
        ...
        # the following function is disabled because it implicitly wires all 'INIT'
        # event inputs to START.COLD -> no control over order of initialization
        # Because of this reason i disabled it for now
        # self.set_up_connections()

        # create object
        self.obj_idx = "{0}:{1}".format(ua_folder.get("idx"), self.fb_name)
        self.obj_path_list, self.obj_path = utils.default_object(
            ua_server,
            self.obj_idx,
            ua_folder.get("path"),
        ...
        var_folder_idx, var_folder_path, var_folder_list = utils.default_folder(
            ua_server, self.obj_idx, self.obj_path, self.obj_path_list, "Variables"
        )
        var_folder = {
            "idx": var_folder_idx,
        ...
        event_folder_idx, event_folder_path, event_folder_list = utils.default_folder(
            ua_server, self.obj_idx, self.obj_path, self.obj_path_list, "Events"
        )
        event_folder = {
                ...
                if element.tag == "InputVars" or element.tag == "OutputVars":
                    for var_declaration in element:  # Var Declarations
                        ...
                        if "OpcUa" in var_declaration.attrib:
                            try:
                                var_idx = "{0}:{1}".format(
                                    self.folders["VarFolder"].get("idx"),
                                    var_declaration.get("Name"),
                                )
                                ua_var = self.ua_server.create_typed_variable(
                                    self.folders["VarFolder"].get("path"),
                                    var_idx,
                                    var_declaration.get("Name"),
            ...
            for element in child:  # Event and Vars Inputs/Outputs
                if element.tag == "EventInputs" or element.tag == "EventOutputs":
                    for event in element:  # Events
                        ...
                        try:
                            var_idx = "{0}:{1}".format(
                                self.folders["EventFolder"].get("idx"),
                                event.get("Name"),
                            )
                            ua_var = self.ua_server.create_typed_variable(
                                self.folders["EventFolder"].get("path"),
                                var_idx,
                                event.get("Name"),
    ...
    def set_up_connections(self):
        if self.opc_ua_type == "DEVICE.SENSOR":
            self.ua_server.config.create_connection(
            ...
            self.ua_server.config.create_connection(
                "{0}.{1}".format(self.fb_name, "INIT_O"),
                "{0}.{1}".format(sleep_fb_name, "SLEEP"),
            )
            self.ua_server.config.create_connection(
                "{0}.{1}".format(sleep_fb_name, "SLEEP_O"),
                "{0}.{1}".format(self.fb_name, "READ"),
            )
            self.ua_server.config.create_connection(
                "{0}.{1}".format(self.fb_name, "READ_O"),
                "{0}.{1}".format(sleep_fb_name, "SLEEP"),
            )
        else:
            self.ua_server.config.create_connection(
