...
class UaObject:
    ...
    def __init__(self, ua_server, ua_folder, fb_name, xml_root):
        ...
        try:
            ...
            logging.error(
                'Invalid function block definition, check {0}.fbt for mistakes'.format(self.fb_name))
                ...
                if element.tag == 'InputVars' or element.tag == 'OutputVars':
                    for var_declaration in element:  # Var Declarations
                        ...
                        if 'OpcUa' in var_declaration.attrib:
                            try:
                                var_idx = '{0}:{1}'.format(self.folders['VarFolder'].get(
                                    'idx'), var_declaration.get('Name'))
                                ua_var = self.ua_server.create_typed_variable(self.folders['VarFolder'].get('path'),
                                                                              var_idx,
                                                                              var_declaration.get(
                                                                                  'Name'),
                                                                              utils.UA_TYPES[var_declaration.get(
                                                                                  'Type')],
                                                                              0)
                                self.ua_vars[var_declaration.get(
                                    'Name')] = ua_var
                            except KeyError:
                                raise self.InvalidFbtState
            ...
            for element in child:  # Event and Vars Inputs/Outputs
                if element.tag == 'EventInputs' or element.tag == 'EventOutputs':
                    for event in element:  # Events
                        ...
                        try:
                            var_idx = '{0}:{1}'.format(
                                self.folders['EventFolder'].get('idx'), event.get('Name'))
                            ...
                            print(
                                'Accessed non existent attrib {0}'.format(ke.args))
                            raise self.InvalidFbtState
    ...
    def update_variables(self):
        ...
        for var_name, var_ua in self.ua_vars.items():
            ...
            try:
                ...
                # reports the error
                logging.warning(
                    'Error writing the value in the opc-ua server.')
                ...
                if v_type == 'STRING':
                    ...
                    # writes the solution
                    logging.warning(
                        'Error solved writing the variable as string.')
