...
class UaMethod:
    def __init__(self, ua_server, ua_folder, xml_root):
        ...
        starting_fb = self.ua_server.config.get_fb(
            self.ua_server.method_event.split(".")[0]
        )
    ...
    def __execute(self, parent, *args):
        ...
        for i, (attr_name, _) in enumerate(self.inputs.items()):
            if attr_name != self.ua_server.method_event:
                ...
                self.ua_server.config.fb_dictionary.get(chunks[0]).set_attr(
                    chunks[1], args[i]
                )

        # push event with user inserted value
        chunks = self.ua_server.method_event.split(".")  # fb_name.event_name
        self.ua_server.config.fb_dictionary.get(chunks[0]).push_event(
            chunks[1], args[len(args) - 1]
        )

        self.ua_server.config.fb_dictionary.get(
            self.ua_server.method_final_fb
        ).execution_end.wait()
        ...
        for i, (attr_name, _) in enumerate(self.outputs.items()):
            chunks = attr_name.split(".")  # fb_name.attr_name
            _, value, _ = self.ua_server.config.fb_dictionary.get(chunks[0]).read_attr(
                chunks[1]
            )
    ...
    def get_fbs(self, fb):
        ...
        for _, connections in fb.output_connections.items():
            for connection in connections:
                string_connection = (
                    connection.destination_fb.fb_name + "." + connection.value_name
                )
    ...
    def add_inputs_outputs(self):
        ...
        if self.ua_server.method_inputs is not None:
            has_inputs = self.interpret_info(
                self.ua_server.method_inputs, self.fbs_inputs
            )

        if self.ua_server.method_outputs is not None:
            has_outputs = self.interpret_info(
                self.ua_server.method_outputs, self.fbs_outputs
            )
    ...
    def generate_inputs_outputs(
        self, fb_name, xml_root, inputs_defined, outputs_defined
    ):
                ...
                if child.tag == "InputVars":
                    for var_declaration in child:
                        if "OpcUa" in var_declaration.attrib:
                            ...
                            if inputs_defined and (
                                fb_name not in self.fbs_inputs
                                or var_name not in self.fbs_inputs[fb_name]
                            ):
                                continue
                            ...
                            _, value, _ = self.ua_server.config.get_fb(
                                fb_name
                            ).read_attr(var_name)
                ...
                if child.tag == "OutputVars":
                    for var_declaration in child:
                        if "OpcUa" in var_declaration.attrib:
                            ...
                            if outputs_defined and (
                                fb_name not in self.fbs_outputs
                                or var_name not in self.fbs_outputs[fb_name]
                            ):
                                continue
                            # gets the type
                            var_type = var_declaration.attrib["Type"]
    ...
    def virtualize(self):
        ...
        self.ua_server.create_method(
            self.ua_folder.get("path"),
            method_idx,
            browse_name,
            self.__execute,
